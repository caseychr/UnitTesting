...
public class NoteRepository {
    ...
    public Flowable<Resource<Integer>> insertNote(final Note note) throws Exception {
        checkTitle(note);
        return mNoteDao.insertNote(note).delaySubscription(timeDelay, mTimeUnit)
    ...
    public Flowable<Resource<Integer>> updatedNote(final Note note) throws Exception {
        checkTitle(note);
        return mNoteDao.updateNote(note).delaySubscription(timeDelay, mTimeUnit)
                .onErrorReturn(new Function<Throwable, Integer>() {
                    @Override
                    public Integer apply(Throwable throwable) throws Exception {
                        return -1;
    ...
    public LiveData<Resource<Integer>> deleteNote(final Note note) throws Exception {
        ...
        return LiveDataReactiveStreams.fromPublisher(mNoteDao.deleteNote(note)
        .onErrorReturn(new Function<Throwable, Integer>() {
            @Override
            public Integer apply(Throwable throwable) throws Exception {
                return -1;
